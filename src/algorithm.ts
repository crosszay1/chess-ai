...
export class Algorithm {
  public decideMove(chess: Chess, depth = 3) {
    ...
    for (const move of moves) {
      ...
      const currentScore = this.miniMax(
        chess,
        depth,
        maxOrMini,
        -Infinity,
  ...
  public evaluateBoard(
    ...
    const blackKingSafetyScore = this.kingSafetyScore(
      chess,
      "b",
      blackKingSquare,
    );
    const whiteKingSafetyScore = this.kingSafetyScore(
      chess,
      "w",
      whiteKingSquare,
    );
  ...
  private mobilityScore(chess: Chess): number {
    ...
    const [placement, active, castling, ep, half, full] = chess
      .fen()
      .split(" ");
    ...
    const otherSideMoves = new Chess(flippedFen, {
      skipValidation: true,
    }).moves().length;
  ...
  private kingSafetyScore(
    ...
    for (let df = -1; df <= 1; df++) {
      for (let dr = -1; dr <= 1; dr++) {
        ...
        const nearbySquare = (String.fromCharCode("a".charCodeAt(0) + newFile) +
          (newRank + 1)) as Square;
    ...
    for (let file = 0; file < 8; file++) {
      ...
      for (let rank = 0; rank < 8; rank++) {
        ...
        if (whitePawnsInFile > 1) {
          // if more then 1 pawn in that file, it's a doubled pawn
  ...
  private scoreMoves(move: Move): number {
    let score = 0;
    if (move.captured)
      score += 10 * pieceValues[move.captured] - pieceValues[move.piece]; // MVV-LVA
